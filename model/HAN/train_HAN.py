...
def main(args):
    ...
    for canonical_etype in graph.canonical_etypes:
        ...
        graph.edges[canonical_etype].data["id"] = (torch.ones(graph.num_edges(canonical_etype), dtype=torch.long) * edge_dict[etype]).to(args['device'])
    
    han = HAN(
        graph,
        node_dict,
        edge_dict,
        n_hid=args['hidden_units'][1],
        n_out=args['hidden_units'][-1],
        n_layers=2,
        n_heads=4,
        use_norm=True,
    ).to(args['device'])
    ...
    # 3) 组合
    model = nn.Sequential(han, classifier)
