...
def load_dgl_data(device, label='view'):
    file_path = '../../data/heterogeneous_graph.pkl'
    graph = dgl.load_graphs(file_path)[0][0].to(device)
    labels = pd.read_csv('../../data/subtopic_label.csv')[label]
    
    subtopic_ids = graph.nodes('subtopic')
    train_nid, others_nid = train_test_split(subtopic_ids.cpu().numpy(), test_size=0.2, random_state=9)
    test_nid, val_nid = train_test_split(others_nid, test_size=0.5, random_state=18)

    labels = torch.tensor(labels.to_numpy()).to(device).unsqueeze(1)
    train_nid, val_nid, test_nid = np.array(train_nid), np.array(val_nid), np.array(test_nid)
    train_nid = torch.tensor(train_nid).to(device)
    val_nid, test_nid = torch.tensor(val_nid).to(device), torch.tensor(test_nid).to(device)

    # feature initilization
    for ntype in ['desc', 'title', 'content', 'comment','video', 'subtopic','topic', 'fans','user']:
        graph.nodes[ntype].data["feat"] = graph.nodes[ntype].data["feat"].to(device)

    for ntype in ['desc', 'title', 'content', 'comment', 'topic', 'video']:
        feature = torch.tensor(np.load(f'/home/qian/chenshangheng/graph360/topic/data/script/{ntype}.npy'))
        graph.nodes[ntype].data["feat"] = feature.float().to(device)


    target_node_type = "subtopic"
    # graph = dgl.add_reverse_edges(graph, ignore_bipartite=True)
    # graph = dgl_add_all_reversed_edges(graph)

    return graph, target_node_type, (train_nid, val_nid, test_nid), labels
