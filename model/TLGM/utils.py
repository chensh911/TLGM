...
def load_dgl_data(device, label='view'):
    ...
    return target_node_type, labels
...
def get_llm_data_loader(dataset, label, seed, batch_size, shuffle=False):
    # 定义保存文件的基本路径
    save_folder = f"../../result/{dataset}/{label}/"
    
    # 创建训练集 DataLoader
    train_dataset = GraphDataset(save_folder, seed, 'train')
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
    
    # 创建验证集 DataLoader
    val_dataset = GraphDataset(save_folder, seed, 'val')
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=shuffle)
    
    # 创建测试集 DataLoader
    test_dataset = GraphDataset(save_folder, seed, 'test')
